...
def list_russian_shares(
    ...
    if InstrumentStatus is not None:
        instrument_status = getattr(
            ...
            getattr(InstrumentStatus, "INSTRUMENT_STATUS_UNSPECIFIED", None),
        )
    ...
    try:
        with Client(token) as client:
            ...
            if instrument_status is not None:
                shares_kwargs["instrument_status"] = instrument_status
            response = client.instruments.shares(**shares_kwargs)
    ...
    for share in shares:
        try:
            if getattr(share, "country_of_risk", "").upper() not in {"RU", "RUS"}:
                continue
            exchange = getattr(share, "exchange", "").upper()
            if exchange not in {"MOEX", "RUS", "SPB"}:
                continue
